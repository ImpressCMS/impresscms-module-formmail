...
class FormmailElementsHandler {
	var $db;
	function FormmailElementsHandler(&$db) {
		$this->db =& $db;
	}
	function &getInstance(&$db) {
		static $instance;
		if (!isset($instance)) {
			$instance = new FormmailElementsHandler($db);
		}
		return $instance;
	}
	function &create() {
	...
	function insert(&$element, $force = false){
        if( (get_class($element) != 'FormmailFormmail') && ( get_class($element) != 'formmailformmail')){
            return false;
        }
        if( !$element->isDirty() ){
            return true;
	...
	function delete(&$element, $force = false){
		if(( get_class($this) != 'formmailelementshandler') && ( get_class($this) != 'FormmailElementsHandler')) {
			return false;
		}
